...
func (m *FileTreeModel) View(width, height int) string {
	...
	if len(m.items) == 0 {
		b.WriteString(helpStyle.Render("No files found or loading..."))
		return b.String()
	}
...
func (m FileTreeModel) renderFileTreeItem(item FileTreeItem, isCursor, isSelected bool) string {
	var b strings.Builder
...
func (m FileTreeModel) getFileIcon(item FileTreeItem) string {
	if item.IsDirectory {
...
func (m FileTreeModel) buildFileTree(path string, level int) ([]FileTreeItem, error) {
...
func (m FileTreeModel) showDirectoryInfo(path string) tea.Cmd {
	return func() tea.Msg {
		...
		if err != nil {
			return StatusUpdateMsg{Message: fmt.Sprintf("Error reading directory: %s", err.Error())}
		}
		
		// Count files in directory
		entries, err := os.ReadDir(path)
		if err != nil {
			return StatusUpdateMsg{Message: fmt.Sprintf("Error reading directory contents: %s", err.Error())}
		}
